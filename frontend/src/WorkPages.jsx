...
export function WorkCalendar({data,params,go,onCreate,demo}) {
 ...
 const [selectedDay,setSelectedDay]=useState(''),[visibleMonth,setVisibleMonth]=useState(todayMonth),wheelLock=useRef(0)
 ...
 const onCalendarWheel=event=>{if(Math.abs(event.deltaY)<12||Date.now()<wheelLock.current)return;event.preventDefault();wheelLock.current=Date.now()+500;shiftMonth(event.deltaY>0?1:-1)}
