...
export function WorkCalendar({data,params,go}) {
 ...
 const months=calendarMonths(today),counts=events.reduce((group,event)=>((group[event.date]??=[]).push(event),group),{})
 const [selectedDay,setSelectedDay]=useState(''),viewport=useRef(null),currentMonth=useRef(null)
 const change=patch=>go('calendar',false,{...Object.fromEntries(params),...patch,id:''})
 const scrollToCurrent=()=>{if(viewport.current&&currentMonth.current){const top=currentMonth.current.getBoundingClientRect().top-viewport.current.getBoundingClientRect().top+viewport.current.scrollTop;viewport.current.scrollTo({top,behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})}}
 useEffect(()=>{if(viewport.current&&currentMonth.current){viewport.current.scrollTop=currentMonth.current.getBoundingClientRect().top-viewport.current.getBoundingClientRect().top+viewport.current.scrollTop}},[])
 const scrollMonth=direction=>viewport.current?.scrollBy({top:direction*viewport.current.clientHeight,behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})
 return <div className="work-page"><div className="page-heading"><div><h1>Lịch quay & họp</h1><p>{events.length} sự kiện · Tháng hiện tại được mở sẵn</p></div></div><section className="panel calendar-toolbar"><Scope data={data} value={scope} onChange={assignee=>change({assignee})}/><div className="calendar-navigation"><button type="button" className="secondary-button" onClick={()=>scrollMonth(-1)} aria-label="Tháng trước">‹ Tháng trước</button><button type="button" className="primary-button" onClick={scrollToCurrent}>Hôm nay</button><button type="button" className="secondary-button" onClick={()=>scrollMonth(1)} aria-label="Tháng sau">Tháng sau ›</button></div></section><section className="panel calendar-panel"><div className="calendar-scroll" ref={viewport}>{months.map(month=><section className="calendar-month" key={month.key} ref={month.key===todayMonth?currentMonth:null}><h2>{month.label}</h2><div className="calendar-grid calendar-weekdays">{['T2','T3','T4','T5','T6','T7','CN'].map(day=><b key={day}>{day}</b>)}</div><div className="calendar-grid calendar-days">{month.days.map((date,index)=>{if(!date)return <span className="calendar-blank" key={`blank-${index}`}/>;const dayEvents=counts[date]||[],dayNumber=Number(date.slice(-2));return <button type="button" key={date} className={`calendar-day ${date===today?'is-today':date<today?'is-past':'is-future'}`} onClick={()=>setSelectedDay(date)} aria-label={`${formatVietnamDate(date)}, ${dayEvents.length} sự kiện`}><span className="calendar-day-heading"><b>{dayNumber}</b>{dayEvents.length>0&&<i>{dayEvents.length}</i>}</span>{dayEvents.slice(0,2).map(event=><span key={event.id} className={`calendar-event-chip ${event.kindLabel==='Quay'?'shoot':event.kindLabel==='Họp'?'meeting':'live'}`}><small>{event.time}</small>{event.title}</span>)}{dayEvents.length>2&&<small className="calendar-more">+{dayEvents.length-2} lịch</small>}</button>})}</div></section>)}</div></section>{selectedDay&&<DetailDialog title={formatVietnamDate(selectedDay)} onClose={()=>setSelectedDay('')}><div className="calendar-day-dialog">{(counts[selectedDay]||[]).map(event=><button key={event.id} type="button" className="calendar-event-row" onClick={()=>{setSelectedDay('');go('calendar',false,{...Object.fromEntries(params),id:event.id})}}><span className={`calendar-event-type ${event.kindLabel==='Quay'?'shoot':event.kindLabel==='Họp'?'meeting':'live'}`}>{event.kindLabel}</span><span><b>{event.title}</b><small>{event.time||'Chưa có giờ'} · {event.location||'Chưa có địa điểm'}</small></span><span aria-hidden="true">›</span></button>)}{!(counts[selectedDay]||[]).length&&<p className="work-empty">Ngày này chưa có lịch. Bạn có thể xem ngày khác trong tháng.</p>}</div></DetailDialog>}{params.get('id')&&<DetailDialog title={detail?.title||'Không tìm thấy sự kiện'} onClose={()=>change({})}>{detail?<dl className="work-details">{[['Loại lịch',detail.kindLabel],['Ngày giờ',`${formatVietnamDate(detail.date)} · ${detail.time||'Chưa có giờ'}`],['Địa điểm',detail.location],['Người chủ trì',data.users.find(u=>u.email===detail.lead)?.name||detail.lead],['Người tham gia',(detail.attendees||[]).map(email=>data.users.find(u=>u.email===email)?.name||email).join(', ')],['Ghi chú',detail.brief||detail.note]].map(([k,v])=><div key={k}><dt>{k}</dt><dd>{v||'Chưa có'}</dd></div>)}</dl>:<p>Sự kiện không tồn tại hoặc bạn không có quyền xem.</p>}</DetailDialog>}</div>
}
