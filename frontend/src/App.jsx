...
function PayrollPage({data,onCompute,onSaveBreakdown,demo}) {
  const rows=data.payroll||[]
  const canManage=!demo && Boolean(data.me?.isAdmin||data.me?.caps?.includes('payroll.compute'))
  const [selectedRow,setSelectedRow]=useState(null)
  return <div><PageHeading eyebrow="TỔNG HỢP THU NHẬP" title="Lương thưởng" description="Kết quả KPI và thu nhập theo kỳ tính lương." action={<button className="secondary-button" disabled title="Chưa có bộ chọn kỳ lương"><Icon name="calendar"/> {data.payrollMonth||'Chưa có kỳ lương'} <span>⌄</span></button>}/><div className="metric-grid three"><Metric label="Tổng quỹ lương" value={money(rows.reduce((s,r)=>s+Number(r.total||0),0))} delta="Kỳ hiện tại" color="purple" icon="₫"/><Metric label="KPI đạt trung bình" value={rows.length?Math.round(rows.reduce((s,r)=>s+Number(r.kpi_rate||0),0)/rows.length)+'%':'—'} delta="Toàn nhóm" color="green" icon="◉"/><Metric label="Thành viên" value={rows.length} delta="Trong kỳ" color="blue" icon="♙"/></div><section className="panel table-panel"><div className="table-toolbar"><h2>Bảng lương tháng {data.payrollMonth||''}</h2><div className="flex gap-2">{canManage&&<button className="secondary-button" onClick={onCompute}>Tính lại từ Policy</button>}<button className="secondary-button" disabled title="Tính năng xuất báo cáo sẽ được bổ sung">Xuất báo cáo <Icon name="arrow"/></button></div></div><div className="simple-table payroll-table"><div className="table-head"><span>THÀNH VIÊN</span><span>LƯƠNG CƠ BẢN</span><span>PHỤ CẤP</span><span>THƯỞNG / PHẠT</span><span>KPI</span><span>TỔNG NHẬN</span></div>{rows.map(r=>{const u=data.users?.find(x=>x.email===r.email);return <div className="table-row" key={r.email}><span className="title-cell"><Avatar user={u}/><button type="button" className="payroll-member-link" onClick={()=>setSelectedRow(r)}>{u?.name||r.email}</button></span><span>{money(r.base)}</span><span>{money(r.fees)}</span><span>{money(Number(r.bonus||0)-Number(r.penalty||0))}</span><span><span className="kpi-mini"><i style={{width:(r.kpi_rate||0)+'%'}}/></span>{Math.round(r.kpi_rate||0)}%</span><b>{money(r.total)}</b></div>})}</div></section>{selectedRow&&<PayrollDetailsModal row={selectedRow} name={data.users?.find(user=>user.email===selectedRow.email)?.name||selectedRow.email} month={data.payrollMonth} canManage={canManage} onSave={onSaveBreakdown} onClose={()=>setSelectedRow(null)}/>}</div>
}

function PayrollDetailsModal({row,name,month,canManage,onSave,onClose}) {
  const [items,setItems]=useState(()=>payrollBreakdown(row))
  const [editing,setEditing]=useState(null)
  const [saving,setSaving]=useState(false)
  const total=items.reduce((sum,item)=>sum+Number(item.amount||0),0)
  const updateItem=(index,patch)=>setItems(current=>current.map((item,i)=>i===index?{...item,...patch}:item))
  const save=async()=>{setSaving(true);try{await onSave(row,items);onClose()}finally{setSaving(false)}}
  return <Modal title={name} onClose={onClose} className="policy-detail-modal"><p className="policy-detail-total">Chi tiết tháng {month||'—'} <b>{money(total)}</b></p>{canManage&&<p className="policy-empty">Nhấp đúp số tiền để sửa; các dòng thêm/xóa sẽ cập nhật tổng. Tính lại từ Policy sẽ ghi đè chỉnh sửa này.</p>}<div className="policy-detail-list">{items.map((item,index)=><article key={`${item.code}-${index}`}><div><b>{canManage?<input aria-label="Tên khoản lương" value={item.label} onChange={e=>updateItem(index,{label:e.target.value})}/>:item.label}</b><small>{item.how||'Chi tiết công thức'}</small></div><div className="flex gap-2">{canManage&&<select aria-label="Nhóm khoản lương" value={item.group} onChange={e=>updateItem(index,{group:e.target.value,amount:e.target.value==='penalty'?-Math.abs(Number(item.amount)):Math.abs(Number(item.amount))})}><option value="base">Lương cơ bản</option><option value="fees">Phụ cấp</option><option value="bonus">Thưởng</option><option value="penalty">Phạt</option></select>}{editing===index?<input aria-label="Số tiền" type="number" step="1" value={item.amount} autoFocus onChange={e=>updateItem(index,{amount:item.group==='penalty'?-Math.abs(Number(e.target.value)):Math.abs(Number(e.target.value))})} onBlur={()=>setEditing(null)} onKeyDown={e=>e.key==='Enter'&&setEditing(null)}/>:<strong onDoubleClick={()=>canManage&&setEditing(index)}>{money(item.amount)}</strong>}{canManage&&<button type="button" className="icon-button" aria-label="Xóa dòng" onClick={()=>setItems(current=>current.filter((_,i)=>i!==index))}>×</button>}</div></article>)}{!items.length&&<div className="policy-empty">Chưa có dòng chi tiết. Có thể thêm khoản lương thủ công.</div>}</div>{canManage&&<div className="flex gap-2" style={{marginTop:12}}><button type="button" className="secondary-button" onClick={()=>setItems(current=>[...current,{code:`manual-${Date.now()}`,label:'Khoản bổ sung',group:'fees',how:'Điều chỉnh thủ công',amount:0}])}>+ Thêm dòng</button><button type="button" className="primary-button" disabled={saving} onClick={save}>{saving?'Đang lưu…':'Lưu thay đổi'}</button></div>}</Modal>
}
